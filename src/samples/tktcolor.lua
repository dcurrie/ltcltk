...
function tc_scaleChanged()
    ...
    if colorspace == 'rgb' then
	    ltk.var.red = string.format("%.0f", scales[1]:get()*65.535)
	    ltk.var.green = string.format("%.0f", scales[2]:get()*65.535)
	    ltk.var.blue = string.format("%.0f", scales[3]:get()*65.535)
	elseif colorspace == 'cmy' then
	    ltk.var.red = string.format("%.0f", 65535 - scales[1]:get()*65.535)
	    ltk.var.green = string.format("%.0f", 65535 - scales[2]:get()*65.535)
	    ltk.var.blue = string.format("%.0f", 65535 - scales[3]:get()*65.535)
	elseif colorspace == 'hsb' then
	    local r, g, b = hsbToRgb(scales[1]:get()/1000.0,
		    scales[2]:get()/1000.0,
		    scales[3]:get()/1000.0)
		ltk.var.red = r
		ltk.var.green = g
    ...
    _sample_swatch:configure{ bg=ltk.var.color }
...
function tc_setScales()
    ...
    if colorspace=='rgb' then
	    scales[1]:set(string.format('%.0f', red/65.535))
	    scales[2]:set(string.format('%.0f', green/65.535))
	    scales[3]:set(string.format('%.0f', blue/65.535))
	elseif colorspace=='cmy' then
	    scales[1]:set(string.format('%.0f', (65535-red)/65.535))
	    scales[2]:set(string.format('%.0f', (65535-green)/65.535))
	    scales[3]:set(string.format('%.0f', (65535-blue)/65.535))
	elseif colorspace=='hsb' then
	    local h, s, v = rgbToHsv(red, green, blue)
	    scales[1]:set(string.format('%.0f', h * 1000.0))
	    scales[2]:set(string.format('%.0f', s * 1000.0))
	    scales[3]:set(string.format('%.0f', v * 1000.0))
	end
...
function tc_loadNamedColor(wname)
	...
	if ltk.widgettype(wname) == 'entry' then
		...
		name = wname:get(wname:curselection())
	end
	local r, g, b
    ...
    _sample_swatch:configure {bg=color}
    if ltk.var.autoUpdate then doUpdate() end
end
...
_menu:add{'cascade', menu=_file, label='File', underline=0}

_file:add{'radio', label="RGB color space", variable='colorSpace',
	value='rgb', underline=0, command={changeColorSpace, 'rgb'}}
_file:add{'radio', label="CMY color space", variable='colorSpace',
	value='cmy', underline=0, command={changeColorSpace, 'cmy'}}
_file:add{'radio', label="HSB color space", variable='colorSpace',
	value='hsb', underline=0, command={changeColorSpace, 'hsb'}}
_file:add{'separator'}
_file:add{'radio', label="Automatic updates", variable='autoUpdate',
	value=1, underline=0}
_file:add{'radio', label="Manual updates", variable='autoUpdate',
	value=0, underline=0}
_file:add{'separator'}
...
_command = ltk.labelframe{text="Command:", padding='1m'}
...
for _, i in ipairs {
	...
	f = io.open(i, 'r')
	if f then
		_names = ltk.labelframe{text="Select:", padding='.1c'}
		...
		_names_s = ltk.scrollbar(_names){ orient='vertical', command = function(...) text:yview(...) end }
		_names_lb:configure{ yscrollcommand = function(y1, y2) sb:set(y1, y2) end }
