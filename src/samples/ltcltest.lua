...
--[[
 ...
 - should cover most of the functionality.
 -
...
-- 21
announce("converting string '1 2 3 4' to table using list2table, result should be a table {1, 2, 3, 4}")
ok, val = pcall(ltcl.list2table, tcl, '1 2 3 4')
if ok then
	check(cmptables(val, {1,2,3,4}))
else
	fail(tostring(val))
...
-- 52
announce("TCalling 'list' with parameter 'a', 'b', 'c', {1,2,3}, should return a table {'a','b','c',1,2,3}")
ok, val = pcall(ltcl.callt, tcl, 'list', 'a', 'b', 'c', {1,2,3})
if ok then
	check(cmptables(val, {'a','b','c',1,2,3}))
else
	fail(tostring(val))
end

-- 53
announce("Making argument list from {1,2,c=3} then TCalling list with 'a', 'b', this list, should return a table {'a','b',1,2,'-c',3}")
ok, args = pcall(ltcl.makearglist, tcl, {1,2,c=3})
ok, val = pcall(ltcl.callt, tcl, 'list', 'a', 'b', args)
if ok then
	check(cmptables(val, {'a','b',1,2,'-c',3}))
else
	fail(tostring(val))
end

-- 54
announce("TCalling 'list' with arguments 1, 2, 3, should return table {1,2,3}")
ok, val = pcall(ltcl.callt, tcl, 'list', 1,2,3)
if ok then
	check(cmptables(val, {1,2,3}))
else
...
-- 55
announce("PTCalling 'list' with argument {1, 2, 3}, should fail")
ok, val = pcall(ltcl.callpt, tcl, 'list', {1,2,3})
check(not ok)

-- 56
announce("PTCalling 'list' with argument {1, 2, 3},4,  should fail")
ok, val = pcall(ltcl.callpt, tcl, 'list', {1,2,3}, 4)
check(not ok)
