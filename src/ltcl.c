...
/* this may need a little more sophistication */
 ...
 * 
 * Reasonably valid means that it contains only valid utf8 byte sequences.
 * Beware that since 2003, Utf8 sequences can be at most 4 bytes long, thus
 * longer sequences will be flagged as error. No attempt is made to check
 * wether these sequences actually represent valid characters, except for the
 * invalid bytes 0xC0, 0xC1 and 0xF5-0xFF.
 * 
 * These patterns from http://en.wikipedia.org/wiki/Utf8 are used:
 ...
 *
 * Note: this also flags a string as not being valid Utf8 if it contains
...
static int ltcl_pushTclObj(lua_State *L, Tcl_Obj *tclobj)
{
	...
	if (tclobj == NULL) {
		...
		*/
		lua_pushinteger(L, tclobj->internalRep.longValue);
	} else if (tclotype == TclByteArrayType) {
		str = (char *) Tcl_GetByteArrayFromObj(tclobj, &len);
		lua_pushlstring(L, str, len);
	/* lists need to be found even if they are returned as strings.
	 */
	} else if (tclotype == TclListType) {
		int i;
...
static Tcl_Obj* ltcl_toTclObj(lua_State *L, int index, lTclPtrList *lst_in)
{
	...
	if (type == LUA_TNIL) {
		...
		return NULL; /* in order to satisfy the return type, should never get herem though */
	}
}
...
/* ltcl_callt
 *
 * call a tcl function with an a prefix of one or more string or number arguments,
 * followed by an array of pre-parsed (i.e. converted from lua) arguments
 *
 * Arguments:
 *	L	Lua State
 *
 ...
 *  +n	n >=0, string arguments
 *	+n+1 table of additional function arguments
 *
 * Lua Returns:
 *	+1	the result of the evaluation
 *
...
static int ltcl_callt(lua_State *L)
{
	...
	/* process additional string arguments */
	while (lua_isstring(L, base) && base <= nargs) {
		o = ltcl_toTclObj(L, base++, NULL);
		_ltcl_ptrlistpush(args, o);
		Tcl_IncrRefCount(o);
	}

	/* optional table with additional args */
	if (base == nargs && !lua_isnoneornil(L, base)) {
 ...
 * Lua Stack:
 *	1	tcl interpreter
 *	2	string representing a tcl list
 *
 * Lua Returns:
 *	+1	table representation of list, or nil+error
 */
static int ltcl_list2table(lua_State *L)
{
	lTcl *interp = ltcl_checkTclInterp(L, 1);
	Tcl_Interp *tcli = interp->interp;
	int len;
	const char *s = luaL_checkstring(L, 2);
	Tcl_Obj *tclo = ltcl_toTclObj(L, 2, NULL);
	if (Tcl_ListObjLength(NULL, tclo, &len) == TCL_OK) {
		return ltcl_pushTclObj(L, tclo);
	}
	return luaL_error(L, "argument is not a list or can not be converted to a table");
}
